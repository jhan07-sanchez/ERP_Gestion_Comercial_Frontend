    ...
    <Card className="border-none shadow-sm ring-1 ring-primary-100 overflow-hidden bg-white">
      <div className="p-5 border-b border-primary-50 flex items-center justify-between">
         <div>
            ...
            <p className="text-xs font-bold text-primary-400 uppercase tracking-widest">Logs recientes del sistema</p>
         </div>
         <button 
            onClick={() => navigate('/auditoria/lista')}
            className="text-xs font-black text-accent-600 hover:underline uppercase tracking-widest"
         >
            Ver Todo
         </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-primary-50/50 border-b border-primary-100">
              <th className="px-5 py-3 text-xs font-black text-primary-400 uppercase tracking-widest">Usuario</th>
              <th className="px-5 py-3 text-xs font-black text-primary-400 uppercase tracking-widest">Acción / Evento</th>
              <th className="px-5 py-3 text-xs font-black text-primary-400 uppercase tracking-widest">Módulo</th>
              <th className="px-5 py-3 text-xs font-black text-primary-400 uppercase tracking-widest text-center">Estado</th>
              <th className="px-5 py-3 text-xs font-black text-primary-400 uppercase tracking-widest text-right">Fecha</th>
          ...
          <tbody className="divide-y divide-primary-50">
            {activities.map((activity) => (
              <tr key={`${activity.type}-${activity.id}`} className="hover:bg-primary-50/50 transition-colors group">
                ...
                <td className="px-5 py-4 whitespace-nowrap">
                   <span className={`px-2 py-1 rounded-lg text-xs font-black uppercase tracking-tighter ${getModuleColor(activity.modulo)}`}>
                      {activity.modulo}
                   </span>
                </td>
                ...
                <td className="px-5 py-4 whitespace-nowrap text-right">
                   <div className="flex flex-col items-end">
                      ...
                      <span className="text-xs opacity-80 font-black text-primary-400 tabular-nums">
