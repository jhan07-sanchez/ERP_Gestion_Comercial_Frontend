...
export function AlertsPanel({ alerts }: AlertsPanelProps) {
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-start justify-between">
                  <h4 className="text-[13px] font-black text-primary-900 leading-tight uppercase tracking-tight">{alert.title}</h4>
                  <span className="text-[9px] font-black text-primary-400 uppercase tracking-widest">
                    {new Date(alert.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                <p className="text-[11px] font-medium text-primary-500 leading-normal">{alert.message}</p>
                
                {alert.actionable && (
                  <div className="flex gap-2 pt-2">
                    <button 
                      onClick={() => navigate(`/compras/nueva?producto_id=${alert.id}`)}
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-primary-900 text-white text-[9px] font-black uppercase tracking-widest hover:bg-primary-800 transition-all shadow-sm active:scale-95"
                    >
                    ...
                    <button 
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-white border border-primary-200 text-primary-600 text-[9px] font-black uppercase tracking-widest hover:bg-primary-50 transition-all shadow-sm"
                    >
                      Ver Producto
                    </button>
                  </div>
      ...
      {alerts.length > 4 && (
        <button className="w-full py-3 rounded-xl border border-dashed border-primary-300 text-[10px] font-black text-primary-400 uppercase tracking-widest hover:text-accent-600 hover:border-accent-400 hover:bg-accent-50/50 transition-all flex items-center justify-center gap-2">
