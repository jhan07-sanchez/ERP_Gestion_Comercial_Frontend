...
export function DashboardHeader({ 
  ...
  const quickActions = [
    { label: "Nueva Venta", icon: <IconShoppingCart size={16} />, path: "/ventas/nueva", color: "bg-blue-600" },
    { label: "Nueva Factura", icon: <IconFileInvoice size={16} />, path: "/facturacion/nueva", color: "bg-indigo-600" },
    { label: "Nuevo Cliente", icon: <IconUserPlus size={16} />, path: "/clientes/nuevo", color: "bg-emerald-600" },
    { label: "Registrar Compra", icon: <IconPlus size={16} />, path: "/compras/nueva", color: "bg-slate-800" },
  ];
    ...
    <div className="flex flex-col gap-6 mb-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
            ...
            <button 
                onClick={onRefresh}
                className="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-500 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition-all shadow-sm group"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {quickActions.map((action) => (
          <button
            ...
            className="flex items-center justify-center gap-2 h-12 rounded-xl bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 hover:bg-blue-50/30 transition-all group overflow-hidden relative"
          >
            ...
            <div className="absolute right-0 top-0 h-full w-1 bg-gradient-to-b from-transparent via-blue-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
          </button>
        ))}
      </div>
    </div>
