...
const PLANES = [
  {
    ...
    buttonColor: 'bg-primary-800 hover:bg-primary-700'
  },
  {
    id: 'PRO',
    name: 'Pro',
...
export default function PlanesPage() {
            ...
            <div className="rounded-md shadow-sm space-y-4">
               <div>
                ...
                <input
                  ...
                  className="appearance-none block w-full px-3 py-2 border border-primary-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                  placeholder="Ej. Juan Pérez"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-primary-700">Nombre de la Empresa</label>
                <input
                  name="empresa"
                  type="text"
                  required
  ...
  return (
    <div className="bg-primary-50 min-h-screen py-16 px-4 sm:px-6 lg:px-8">
      <div className="text-center max-w-3xl mx-auto mb-16">
        <h1 className="text-base text-primary-600 font-semibold tracking-wide uppercase">Precios</h1>
        <p className="mt-2 text-4xl font-extrabold text-primary-900 sm:text-5xl sm:tracking-tight lg:text-6xl">
      ...
      <div className="max-w-7xl mx-auto grid gap-8 lg:grid-cols-3 lg:gap-12">
        {PLANES.map((plan) => (
          <div key={plan.id} className="relative p-8 bg-white border border-primary-200 rounded-2xl shadow-sm flex flex-col hover:shadow-xl transition-shadow duration-300">
            {plan.id === 'PRO' && (
            ...
            <div className="flex-1">
              ...
              <p className="mt-6 text-primary-500">{plan.description}</p>
              <ul className="mt-6 space-y-6">
                {plan.features.map((feature, index) => (
                  <li key={index} className="flex">
                    <svg className="flex-shrink-0 w-6 h-6 text-success-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
                    <span className="ml-3 text-primary-500">{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
