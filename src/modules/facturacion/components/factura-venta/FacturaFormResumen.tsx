...
interface FacturaFormResumenProps {
  value: FacturaFormState;
  submitting: boolean;
...
export function FacturaFormResumen({
  value,
  submitting,
  impuestoPorcentaje,
  ...
  moneda,
  onCancel,
}: FacturaFormResumenProps) {
  const subtotal = value.subtotal || 0;
  const descuento = value.descuento_total || 0;
  const baseImponible = subtotal - descuento;
  const impuestos = baseImponible * (impuestoPorcentaje / 100);
      ...
      <Card className="border-none shadow-xl bg-white text-primary-900 overflow-hidden ring-1 ring-primary-200 sticky top-6">
        <div className="bg-gradient-to-br from-accent-600 to-accent-700 p-6 border-b border-accent-500/10 text-white">
          <div className="flex items-center gap-3 mb-2 opacity-90">
            <IconReceipt size={18} />
            <span className="text-xs font-black uppercase tracking-widest">Resumen de Factura</span>
          </div>
        ...
        <Card.Content className="p-8 space-y-8">
          <div className="space-y-4">
            <div className="flex justify-between items-center text-primary-500">
              ...
              <span className={`font-bold text-primary-700 ${numberClass}`}>{formatCurrency(subtotal)}</span>
            </div>
            <div className="flex justify-between items-center text-primary-500">
            ...
            <div className="flex justify-between items-center text-primary-500">
              ...
              <span className={`font-bold text-primary-700 ${numberClass}`}>{formatCurrency(descuento)}</span>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-xs font-black text-accent-600 uppercase tracking-widest leading-none mb-1">Total</p>
          ...
          <div className="pt-2">
            {value.cliente_id === 0 && value.detalles.length > 0 && (
              <div className="flex items-start gap-3 p-3 bg-warning-50 border border-warning-100 rounded-xl animate-pulse">
                ...
                <p className="text-xs font-bold text-warning-800 leading-relaxed uppercase tracking-tighter">
                  Acción Requerida: Se debe seleccionar un cliente para guardar la factura.
                </p>
              </div>
            )}
          </div>
