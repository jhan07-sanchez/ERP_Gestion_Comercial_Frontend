...
import type { NotaCredito, EmitirNotaCreditoPayload } from "../types/notaCredito.types";
import { EmitirNotaCreditoModal } from "../components/NotasCredito/EmitirNotaCreditoModal";
import { AnularNotaCreditoModal } from "../components/NotasCredito/AnularNotaCreditoModal";

export default function NotaCreditoDetalle() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { showAlert } = useAlert();
  const { emitirNota, anularNota, isSubmitting } = useNotaCreditoActions();
  ...
  const fetchDetalle = async () => {
    setIsLoading(true);
    try {
      const data = await notasCreditoAPI.getNotaById(Number(id));
      setNota(data);
    } catch {
      showAlert("Error", "error", { description: "No se pudo cargar el detalle de la nota de crédito." });
  ...
  const handleEmitirConfirm = async (payload: EmitirNotaCreditoPayload) => {
    const success = await emitirNota(nota.id, payload);
    if (success) {
  ...
  return (
    <PageContainer>
      <PageHeader
        title={`Nota de Crédito ${nota.numero || `#Borrador-${nota.id}`}`}
        subtitle={`Factura Origen: ${nota.factura_numero || `#${nota.factura}`}`}
        icon={<IconFileInvoice size={24} />}
        backButton={
          <Button variant="ghost" onClick={() => navigate("/facturacion/notas_credito/lista")} className="mb-4">
        ...
        actions={
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            {nota.estado === "BORRADOR" && (
              <Button onClick={() => setIsEmitirModalOpen(true)} disabled={isSubmitting}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <Card.Header>
              <h3 className="text-lg font-bold text-primary-900">Líneas de la Nota de Crédito</h3>
            </Card.Header>
      ...
      <EmitirNotaCreditoModal
        isOpen={isEmitirModalOpen}
        onClose={() => setIsEmitirModalOpen(false)}
        onConfirm={handleEmitirConfirm}
        isLoading={isSubmitting}
      />

      <AnularNotaCreditoModal
