...
export default function ProductosList() {
  ...
  const { productos, isLoading, error, fetchProductos, deleteProducto, applyFilters } = useProductos();
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => navigate('/inventario/productos/crear')}>Nuevo Producto</Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => navigate('../productos/crear', { relative: 'route' })}>Nuevo Producto</Button>
      </div>
        ...
        <Card.Content className="overflow-x-auto">
          {productos.length === 0 ? (
            <div className="text-center py-8">
              ...
              <Button className="mt-4" onClick={() => navigate('../productos/crear', { relative: 'route' })}>
              ...
              <tbody>
                {productos.map((producto) => (
                  <tr key={producto.id} className="border-b hover:bg-gray-50">
                    <td className="py-3 px-4 text-gray-700">{producto.codigo}</td>
                    <td className="py-3 px-4 text-gray-900 font-medium">{producto.nombre}</td>
                    <td className="py-3 px-4 text-gray-600">
                      {typeof producto.categoria === 'object' ? producto.categoria.nombre : 'Sin categoría'}
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <Button
                          size="sm"
                          onClick={() => navigate(`../productos/${producto.id}/editar`, { relative: 'route' })}
                        >
                          Editar
                        </Button>
                        <Button size="sm" variant="danger" onClick={() => handleDelete(producto.id)}>
                          Eliminar
                        </Button>
                      </div>
                    </td>
