        ...
        return (
          <PageContainer>
            <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-slate-600 font-black uppercase tracking-widest text-[10px] animate-pulse">Cargando catálogo de categorías...</p>
            </div>
          </PageContainer>
        );
    }
    ...
    return (
        <PageContainer>
            <PageHeader
                ...
                actions={
                  <Button
                    onClick={() => showAlert("En Desarrollo", "info", { description: "La función de crear categoría estará disponible pronto." })}
                    ...
                    <Card.Content className="p-0 overflow-x-auto">
                        {filteredCategorias.length === 0 ? (
                            <div className="text-center py-16 px-4">
                                ...
                                <p className="text-slate-600 font-black uppercase tracking-tight mb-2">No se encontraron categorías</p>
                                <p className="text-xs text-slate-500 font-medium">Revisa el término de búsqueda o crea una nueva.</p>
                            </div>
                        ) : (
                                ...
                                <Table>
                                    <thead>
                                        <tr className="bg-slate-50/50 border-b border-slate-100">
                                            <th className="text-left py-4 px-6 text-[10px] font-black uppercase tracking-widest text-slate-500 w-1/3">Nombre</th>
                                            <th className="text-left py-4 px-6 text-[10px] font-black uppercase tracking-widest text-slate-500 hidden sm:table-cell">Descripción</th>
                                            <th className="text-center py-4 px-6 text-[10px] font-black uppercase tracking-widest text-slate-500 w-32">Estado</th>
                                            <th className="text-center py-4 px-6 text-[10px] font-black uppercase tracking-widest text-slate-500 w-32">Acciones</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {filteredCategorias.map((categoria) => (
                                            <tr key={categoria.id} className="hover:bg-blue-50/30 transition-colors group">
                                                <td className="py-4 px-6">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="flex flex-col min-w-0">
                                                            <span className="font-black text-slate-900 truncate">{categoria.nombre}</span>
                                                ...
                                                <td className="py-4 px-6 hidden sm:table-cell text-sm text-slate-600 font-medium">
                                                    {categoria.descripcion || <span className="text-slate-400 italic">—</span>}
                                                </td>
                                                <td className="py-4 px-6 text-center">
                                                    <Badge variant={categoria.estado ? "success" : "danger"} className="uppercase tracking-widest text-[9px] font-black">
                                                ...
                                                <td className="py-4 px-6 text-center">
                                                    <div className="flex items-center justify-center gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                                                        <button
                                                            onClick={() => showAlert("En Desarrollo", "info", { description: "La función de editar estará disponible pronto." })}
