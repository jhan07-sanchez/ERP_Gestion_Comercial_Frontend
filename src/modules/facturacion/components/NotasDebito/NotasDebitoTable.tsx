...
export function NotasDebitoTable({ notas, isLoading }: NotasDebitoTableProps) {
  ...
  if (isLoading && notas.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-primary-600 font-medium">Cargando notas de débito...</p>
      </div>
    );
  }
  ...
  if (notas.length === 0) {
    return (
      <div className="text-center py-16 px-4">
        <h3 className="text-lg font-bold text-primary-900 mb-1">Sin notas de débito</h3>
            ...
            <Table.Row key={nota.id} className="group hover:bg-primary-50/30 transition-colors">
              ...
              <Table.Cell className="text-center">
                <Button
                  ...
                  variant="secondary"
                  className="px-3 h-8"
                  onClick={() => navigate(`/facturacion/notas_debito/${nota.id}/detalle`, { relative: "route" })}
                >
                  Ver
                </Button>
              </Table.Cell>
